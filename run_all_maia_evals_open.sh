...
RESULTS_DIR="${ROOT_DIR}/judge_results/open_models"
...
start_vllm_server() {
  ...
  case "${model_name}" in
    "google/gemma-3-27b-it")
      # Configurazione validata localmente per Gemma 27B.
...
run_eval() {
  ...
  output_file="${RESULTS_DIR}/eval_${lang}_${prompt_kind}_${dataset_slug}_${model_slug}.json"
...
for model in "${MODELS[@]}"; do
  ...
  run_eval "${model}" "ita" "giobin/MAIA_dev_set_ita_2wrong" "simple" "${ROOT_DIR}/prompts/judge_prompt_ita.txt"
  run_eval "${model}" "eng" "giobin/MAIA_dev_set_eng_2wrong" "simple" "${ROOT_DIR}/prompts/judge_prompt_en.txt"
  run_eval "${model}" "spa" "giobin/MAIA_dev_set_spa_2wrong" "simple" "${ROOT_DIR}/prompts/judge_prompt_es.txt"
